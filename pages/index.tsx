      ...
      <main
        ...
        {isPlaying && (
          <div className='top-0 absolute h-full w-full z-40'>
            ...
            <iframe sandbox="allow-same-origin allow-scripts allow-forms allow-popups" allowFullScreen={true} className='h-full w-full bg-black/75' src={!tv ? `https://www.2embed.cc/embed/${slides.filter((slide) => slide.id === active).map((slide) => slide.id)}` : `https://www.2embed.cc/embed/${slides.filter((slide) => slide.id === active).map((slide) => slide.id)}&s=${season}&e=${episode}`} />
          </div>)}
      </main>
    </React.Fragment>
  )
